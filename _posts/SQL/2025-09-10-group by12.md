...
GROUP BY TO_CHAR(O.SALES_DATE, 'YYYY'), TO_NUMBER(TO_CHAR(O.SALES_DATE, 'MM')), GENDER
...
- 판매일에서 월을 2자리 문자(`'01'`, `'02'`, ...)로 추출한 뒤 숫자형으로 변환한다.
- 예) `01 -> 1`, `02 -> 2`
- 이 처리를 통해 정렬 시 `'10' < '2'` 같은 문제가 없도록 숫자형 정렬이 가능하다.
